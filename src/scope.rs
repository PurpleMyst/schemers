...
impl Scope {
    ...
    fn eval_symbol(&self, symbol: &str) -> Option<GcCell<Value>> {
        self.variables.get(symbol)
            .cloned()
            .or_else(|| self.parent.as_ref().and_then(|parent| parent.eval_symbol(symbol)))
    }
    ...
    pub fn eval(&mut self, value: GcCell<Value>) -> GcCell<Value> {
        match &*value.borrow_mut() {
            ...
            Value::SExpr(contents) => {
                // XXX: This is an hack until I use something other than a Vec for SExpr.
                let func = self.eval((&contents[0]).clone());
