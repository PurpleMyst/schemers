...
named!(string<Value>,
    map!(delimited!(char!('"'), many0!(none_of!("\"")), char!('"')), |x| Value::String(x.into_iter().collect())));
...
mod tests {
    ...
    #[test]
    fn test_string() {
        assert_eq!(string(br#""hello, world!""#).unwrap().1,
                   Value::String("hello, world!".to_owned()));
    }

    #[test]
    fn test_number() {
        assert_eq!(number(b"123 ").unwrap().1,
                   Value::Number(123f64));

        assert_eq!(number(b"42.69 ").unwrap().1,
                   Value::Number(42.69));

        assert_eq!(number(b"1e6 ").unwrap().1,
                   Value::Number(1e6));

        assert_eq!(number(b"-5.123e3 ").unwrap().1,
    ...
    fn test_symbol() {
        ...
        assert_eq!(symbol(b"even-p ").unwrap().1, Value::Symbol("even-p".to_owned()));
    ...
    #[test]
    fn test_sexpr() {
        assert_eq!(sexpr(br#"(print -5.123e3 "hello, world" (add 41 1))"#).unwrap().1,
                   Value::SExpr(vec![Value::Symbol("print".to_owned()),
                                     Value::Number(-5.123e3),
                                     Value::String("hello, world".to_owned()),
                                     Value::SExpr(vec![Value::Symbol("add".to_owned()), Value::Number(41f64), Value::Number(1f64)])]));
    }
}
